...
Name = "runs/DDQN with PER lr=4.8e-2 tau=1e-2 gamma 0.995 {}".format(int(time.time()))
...
################################################################################################
#save the model weights
torch.save(agent.qnetwork_local.state_dict(), 'checkpoints/checkpoint_qnetwork_local_WMT_DDQN_PER.pth')
torch.save(agent.qnetwork_target.state_dict(), 'checkpoints/checkpoint_qnetwork_target_WMT_DDQN_PER.pth')
################################################################################################
...
dataset_gp_test = pd.read_csv('datasets/WMT Historical Data 2011.csv')
