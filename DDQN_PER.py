...
UPDATE_EVERY_LR = 250   # how often to update the learning rate scheduler
UPDATE_EVERY_TARGET = 10  # how often update the target network
...
class Agent():
    ...
    def __init__(self, state_size, action_size, seed, lr_decay=0.999):
        ...
        self.t_step = 0
        # Initialize time step learning rate scheduler(for updating every UPDATE_EVERY_LR steps)
        self.t_step_lr = 0
        # Initialize time step (for updating every UPDATE_EVERY_TARGET steps)
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        # Learn every UPDATE_EVERY_LR time steps.
        self.t_step_lr = (self.t_step_lr + 1) % UPDATE_EVERY_LR
        if self.t_step == 0:
    ...
    def learn(self, experiences, gamma):
        ...
        loss.backward()
        self.optimizer.step()
        self.t_step_target = (self.t_step_target + 1) % UPDATE_EVERY_TARGET
        if self.t_step_lr == 0:
            self.lr_scheduler.step()

        # ------------------- update target network ------------------- #
        if self.t_step_target == 0:  # update target network after every UPDATE_EVERY_TARGET
