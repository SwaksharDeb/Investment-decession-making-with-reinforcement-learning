...
#agent.qnetwork_local.load_state_dict(torch.load('checkpoint_qnetwork_local_WMT_V4.pth'))
#agent.qnetwork_target.load_state_dict(torch.load('checkpoint_qnetwork_target_WMT_V4.pth'))
...
def dqn(n_episodes=5000, max_t=len(data_gp)-1, eps_start=1, eps_end=0.001, eps_decay=0.995):
    ...
    for episode in range(1, n_episodes+1):
        ...
        for t in range(10,max_t):
            ...
            reward = -10000
            change_percent_stock = ((data_gp[t]-statistics.mean(data_gp[t-5:t]))/statistics.mean(data_gp[t-5:t]))*100
...
torch.save(agent.qnetwork_local.state_dict(), 'checkpoint_qnetwork_local_WMT_V5.pth')
torch.save(agent.qnetwork_target.state_dict(), 'checkpoint_qnetwork_target_WMT_V5.pth')
################################################################################################
# plot the scores
fig = plt.figure()
...
agent.qnetwork_local.load_state_dict(torch.load('checkpoint_qnetwork_local_WMT_V5.pth'))
agent.qnetwork_target.load_state_dict(torch.load('checkpoint_qnetwork_target_WMT_V5.pth'))

device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

#import the test data
dataset_gp_test = pd.read_csv('WMT Historical Data 2019.csv')
