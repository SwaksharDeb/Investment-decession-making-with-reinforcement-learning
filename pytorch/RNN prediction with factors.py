...
dataset_train = pd.read_csv('GRAE Historical Data 2007-2017 practice.csv')
...
dataset_test = pd.read_csv('SQPH Historical Data2017-2018 practice.csv')
test_set = dataset_test.iloc[15:, [7,11,12,13,14]].values
test_set_y = dataset_test.iloc[15:, 1:2].values
...
y_test = sc_1.inverse_transform(y_test)

# Visualising the results
plt.plot(y_test, color = 'red', label = 'Real SQPH')
...
model.load_weights("SQPH prediction.json.h5")
...
y_test_1 = []
for i in range(60,len(test_set_y)):
    y_test_1.append(test_set_y[i,0])

y_test_1 = np.array(y_test_1)
    
pred_value = sc.inverse_transform(model.predict(X_test_1))
predicted_stock_price = sc.inverse_transform(model.predict(X_test_1))
