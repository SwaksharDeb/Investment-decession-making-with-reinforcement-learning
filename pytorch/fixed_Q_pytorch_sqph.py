...
classifier.load_weights("sqph classification with factor.json.h5")

#loading the dataset
dataset_2 = pd.read_csv('SQPH Historical Data2018 practice.csv')
dataset_1 = pd.read_csv('SQPH Historical Data 2009-2017.csv')
...
# load weights into new model
regressor.load_weights("sqph prediction with factor.json.h5")
...
scores = dqn()
torch.save(agent.qnetwork_local.state_dict(), 'checkpoint_qnetwork_local_sqph.pth')
torch.save(agent.qnetwork_local.state_dict(), 'checkpoint_qnetwork_target_sqph.pth')
